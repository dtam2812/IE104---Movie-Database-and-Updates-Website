...
function applyUrlParams() {
  ...
  if (params.type) {
    ...
    let targetType = document.querySelector(
  ...
  if (params.genre) {
    ...
    let targetGenre = document.querySelector(
  ...
  if (params.country) {
    ...
    let targetCountry = document.querySelector(
...
function displayMovies(movieList, type) {
  ...
  for (let i = 0; i < movieList.length; i++) {
    ...
    let template = isMovie ? movieCardTemplate : tvCardTemplate;
...
function updatePageNumber() {
  document.querySelector(".pagination__main span:last-child").textContent =
    "/ " + totalPages;
...
filterBtn.addEventListener("click", function (e) {
  ...
  // Lấy cách sắp xếp
  let selectedArrange = document.querySelector(
    ".filter__select-list.arrange .filter__select--active"
  );
...
rightPag.addEventListener("click", function (e) {
  if (currentPage < totalPages) {
    ...
    document.querySelector(".pagination-page-current").textContent =
      currentPage;
    render();
  }
});
