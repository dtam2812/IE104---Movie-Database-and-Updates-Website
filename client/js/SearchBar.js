...
export function searchBar() {
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      ...
      const query = input.value.trim();
      if (query)
        window.location.href = `../pages/SearchPage.html?query=${encodeURIComponent(
          query
        )}`;
    }
  });
    ...
    results.forEach((item) => {
      ...
      card.addEventListener("click", () => {
        ...
        if (item.media_type === "movie")
          window.location.href = `../pages/MovieDetail.html?id=${item.id}`;
        else if (item.media_type === "tv")
          window.location.href = `../pages/TvShowDetail.html?id=${item.id}`;
        else if (item.media_type === "person")
          window.location.href = `../pages/CastDetail.html?id=${item.id}`;
      });
