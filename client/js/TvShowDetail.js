...
async function fetchTvDetails(tvId) {
  ...
  try {
    ...
    // Tiêu đề
    document.querySelector(".detail__title h3").textContent = displayTitle || "Không rõ";
  ...
  return `
    <div class="season">
      ...
      <div class="season__info">
        ...
        ${rating ? `<p class="season__badge">IMDb <span>${rating.toFixed(1)}</span></p>` : ""}
        <p><strong>${t("detail.airDate") || "Ngày phát sóng"}:</strong> ${season.air_date || "N/A"}</p>
        <p><strong>${t("detail.episodes") || "Số tập"}:</strong> ${season.episode_count || "N/A"}</p>
        <p><strong>${t("detail.intro") || "Giới thiệu"}:</strong> ${season.overview || "N/A"}</p>
      </div>
    </div>
  `;
}
...
function renderActors(actors) {
  ...
  if (!actors.length) {
    actorContainer.innerHTML = `<p>${t("detail.noActors") || "Không có thông tin diễn viên."}</p>`;
  ...
  if (viewMoreBtn) {
    ...
    viewMoreBtn.textContent = remain > 0 
...
function renderInfo(tv) {
  ...
  const firstAirDate = tv.first_air_date ? new Date(tv.first_air_date).toLocaleDateString("vi-VN") : "N/A";
  ...
  infoGrid.innerHTML = `
    <h3 class="tab-panel__title">${t("detail.infoTitle") || "Thông tin phim"}</h3>
    <div class="info-item"><div class="info-item__label">${t("tvshow.seasons") || "Số mùa"}:</div><div class="info-item__value">${tv.number_of_seasons || "N/A"}</div></div>
    <div class="info-item"><div class="info-item__label">${t("tvshow.totalEpisodes") || "Tổng số tập"}:</div><div class="info-item__value">${tv.number_of_episodes || "N/A"}</div></div>
    <div class="info-item"><div class="info-item__label">${t("detail.country") || "Quốc gia"}:</div><div class="info-item__value">${flagHTML}</div></div>
    <div class="info-item"><div class="info-item__label">${t("detail.status") || "Trạng thái"}:</div><div class="info-item__value">${tv.status || t("common.unknown")}</div></div>
  `;
}
...
function renderSeasons(seasons) {
  ...
  if (!validSeasons.length) {
    container.innerHTML = `<p>${t("detail.noSeasons") || "Không có thông tin mùa phim."}</p>`;
    if (viewMoreBtn) viewMoreBtn.style.display = "none";
    return;
  }
...
function renderProducers(producers) {
  ...
  container.innerHTML = "";
  if (!producers?.length) {
    container.innerHTML = `<p>${t("detail.noProducers") || "Không có thông tin nhà sản xuất."}</p>`;
    return;
  }
  ...
  producers.forEach(company => {
    ...
    if (logo) {
      container.insertAdjacentHTML("beforeend", `<div class="producer"><img class="producer__logo" src="${logo}" alt="${company.name}" title="${company.name}" /></div>`);
    } else {
      container.insertAdjacentHTML("beforeend", `<div class="producer"><p class="producer__name"><strong>${company.name}</strong></p></div>`);
    }
  });
}
...
async function loadRecommendedTvShows(tvId) {
  ...
  try {
    const res = await fetch(`${BASE_URL}/tv/${tvId}/recommendations?api_key=${TMDB_API_KEY}&language=${apiLang}&page=1`);
    const data = await res.json();
    const shows = (data.results || []).slice(0, 12);

    container.innerHTML = "";
    if (!shows.length) {
      container.innerHTML = `<p>${t("detail.noRecs") || "Không có TV Show đề xuất."}</p>`;
      return;
    }
    ...
    for (const show of shows) {
      ...
      const poster = show.poster_path ? `${IMG_URL}${show.poster_path}` : "https://placehold.co/300x450/1a1a2e/0891b2?text=No+Poster";
...
function initViewMore(buttonSelector, contentSelector) {
  ...
  btn.addEventListener("click", (e) => {
    ...
    toShow.forEach(item => content.insertAdjacentHTML("beforeend", isActor ? createActorHTML(item) : createSeasonHTML(item)));

    content.classList.toggle("expanded");
    const remain = data.length - (isActor ? 5 : 3);
    btn.textContent = isExpanded
      ? `${t("detail.viewMore") || "Xem thêm"} ${remain > 0 ? `(${remain})` : ""} ⮟`
